...
This means: after editing files, leave staging and committing entirely to the user. Don't stage "just the file you touched", don't propose a commit message unprompted.
...
A scalar `type: Tool` and a flow array `type: [Tool, Archived]` are also accepted and coerced to a list, but block style is the canonical form. Canonical content types live in [content/index.md](content/index.md): `Tool` / `How To` / `Approach` / `Course`. Any other string renders as a generic neutral pill. `Tool` and `Approach` are **unique per subject**; the rest can repeat.
...
- **Inline code**: write them as plain inline code — `` `/command` `` — never bold-wrapped code like `` **`/command`** ``.
...
Inline live-search input rendered in the TopBar between `BackLink` and the icon cluster. Component: [Search.tsx](quartz/components/Search.tsx); client logic: [scripts/search.inline.ts](quartz/components/scripts/search.inline.ts); styles: [styles/search.scss](quartz/components/styles/search.scss).

The FlexSearch index is built **once per session** in the browser on the first `nav` event from the global `fetchData` Promise (the `static/contentIndex.json` emitted by `Plugin.ContentIndex` in [quartz.config.ts](quartz.config.ts)). The `IndexHandle` (FlexSearch `Document` + `byId` map) is held in module scope so it survives Quartz's micromorph SPA navigation.

FlexSearch config: `Document` index, `tokenize: forward` on `title` and `content`, `strict` on `tags`. Per-field `encode` is a custom lowercasing tokenizer that splits on whitespace/punctuation but **preserves Unicode letters** — so Czech diacritics are significant by design (`cestina` does **not** match `čeština`). If you ever change this, also update the contract test described in the verification section of the original plan.

Result ranking: title-field hit = 3, tags = 2, content = 1, merged by id and sorted desc; top `maxResults` (default 8) rendered.

UX is **inline dropdown**, not the upstream Quartz Cmd+K modal. Key behaviors:

- `/` (slash) focuses the input from anywhere on the page, unless focus is already in an `<input>` / `<textarea>` / `[contenteditable]`.
- `ArrowUp` / `ArrowDown` cycle the active result (`aria-selected="true"`).
- `Enter` opens the active result (or first if none).
- `Esc` clears the input and blurs.
- Outside-click closes the dropdown.

SPA gotchas covered:

- `<input>.value` is reset on every `nav` event (the same morph-survives-but-doesn't-reset gotcha called out under "SPA morph navigation & form state" below for DatabaseView filters).
- All event listeners use a per-bind `AbortController` registered with `window.addCleanup(() => ac.abort())`.
- The `nav`-event handler is the bind point; the index Promise is module-scoped so the second nav re-uses the already-built index.
...
[scripts/databaseView.inline.ts](quartz/components/scripts/databaseView.inline.ts) normalizes every filter via `resetSelectToDefault` / `resetInputToDefault` at the top of each `bind()`, reading `<option>.defaultSelected` and `<input>.defaultValue` (which always reflect the _current_ page's SSR intent). These must run **before** `applyArchiveContextOverride` and the first `applyFilters()`. Add matching resets for any new filter control.
...
**Filter state ↔ URL query params.** [scripts/databaseView.inline.ts](quartz/components/scripts/databaseView.inline.ts) mirrors live filter values to the URL via `history.replaceState` (`?name=…&type=…&tag=…&url=…`); only non-default values are emitted, using the same `<input>.defaultValue` / `<option>.defaultSelected` primitives the resets read. Precedence at `bind()` time: **reset to defaults → `readFiltersFromURL` overlays params → `applyArchiveContextOverride` only if no URL params were applied.** An explicit URL (bookmark, history.back restoration) always wins over inferred archive context. Companion: [scripts/backLink.inline.ts](quartz/components/scripts/backLink.inline.ts) uses `window.history.back()` instead of navigating to the static `href` when the user arrived via SPA navigation (tracked by the same `previousPath` signal), so the browser restores the filtered URL plus scroll position naturally; the `<a href>` fallback covers direct-entry-to-article (external referrer, opened-in-new-tab).
