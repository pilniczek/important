...
**Dimensions are pinned globally** — the internal chart size (`chartWidth`/`chartHeight: 500`) is set in the Mermaid `quadrantChart` config in **both** the build-time renderer ([ofm.ts](quartz/plugins/transformers/ofm.ts)) and the client theme-toggle re-render ([mermaid.inline.ts](quartz/components/scripts/mermaid.inline.ts)) — keep the two in sync so they can't drift. The rendered on-page SVG display size is capped in [custom.scss](quartz/styles/custom.scss). Adjust there for a different standard size, not per-page.
...
**`xychart-beta` styling is per-article, not global** — opt in via a `cssclass`, the same pattern as the matrix tints above. The site's only instance is [content/diffusion-of-innovations.md](content/diffusion-of-innovations.md) (the diffusion bell/S-curve, `cssclasses: [diffusion-curve]`); its full mechanics — render-time geometry/axis-chrome config in [ofm.ts](quartz/plugins/transformers/ofm.ts) + [mermaid.inline.ts](quartz/components/scripts/mermaid.inline.ts) (which must stay in sync), visual-only tweaks scoped under `.diffusion-curve` in [custom.scss](quartz/styles/custom.scss), and a preview-only in-source `%%{init}%%` directive — are documented at those sources. A future `xychart` starts from Mermaid's defaults and opts into whichever it wants via its own `cssclass`.
