...
- `/init` - Scans the current repo and generates a `CLAUDE.md` briefing. Keep the result tiny or do not use it anyway: [[never-run-claude-init|never run claude /init]].
...
- `/statusline` - Configures the bottom-of-terminal status line (model, branch, token usage, etc.). See [[claude-statusline|claude-statusline]] for a custom replacement scaled to a 100k-token smart zone.
...
- `/code-review` - Reviews the current diff for bugs and cleanups at a chosen effort level. Add `--comment` to post inline PR comments, `--fix` to apply findings to the working tree.
- `/security-review` - Security-focused review of the pending changes on the current branch.
- `/verify` - Runs the app and observes real behaviour to confirm a change actually works - not just that tests pass.
- `/deep-research` - Fan-out web research: searches many sources, adversarially verifies claims, and synthesises a cited report. Narrow the question first if it's vague.

## MCP servers

- `/mcp` - Lists configured MCP servers and their connection status; authenticate OAuth servers and inspect the tools/resources each exposes.
