...
- **`[remote "origin"] push = HEAD`** → [Push current branch by default](../push-head/index.md)
- **`[pull] rebase = true`** → [Pull without merge commits](../pull-rebase/index.md)
- **`[rerere] enabled = true`** → [Remember conflict resolutions](../git-rerere/index.md)
- **`[core] autocrlf` / `safecrlf`** → [Safe line endings](../autocrlf/index.md)
- **`[alias] pf = push --force-with-lease`** → [Safe force push](../force-push-vs-force-with-lease/index.md)
- **`[commit] gpgsign = true` / `[user] signingkey`** → [Sign your commits](../gpg-sign-commits/index.md)
