...
**Git Graph on startup.** For the `git-graph.view` rule to match, the `.git` folder must be visible to the glob - set `"**/.git": false` in `files.exclude` (see [[vscode-setup|VSCode Setup]]). `workbench.startupEditor: "none"` + `window.restoreWindows: "all"` then keep the Welcome tab from covering the auto-opened view.
