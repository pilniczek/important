---
title: Modern Web Builders or “classy” code
tags:
  - Architecture
type: Approach
---
...
1. Web pages generated in real time
2. Using server-side languages (PHP, Node.js, Java, ASP, etc.)
3. Change content according to client request
4. Access information from a database
5. Used for: e-commerce, social media, forums
...
Pages are not hosted on the server in finished form but are assembled for each new request: first the server finds the right document and sends it to the interpreter, which executes the code and consults the database. The document is then returned and rendered in the browser.

## Static websites

Properties:

1. Content on each page is fixed and won't change until it is edited and republished
2. Using client-side languages only (HTML, CSS, JS)
3. Unchanged content until the source code changes
4. No database access
5. Used for: small, simple websites which don't require continuous changes

A static site consists of unchanging pages that look the same for every visitor. When such a site is requested in a browser, the server immediately provides the finished HTML document in its original form. This makes them easy to build and fast to load.
...
## W**hy is it sometimes better to use them?**
...
4. I**t is also worth understanding the size of this project, a business card website for a web builder is a simple task, but something big would be better to implement in a “classy” way.**
...
6. Most web builders charge a monthly fee — factor that into the project budget.

If you do not follow these couple of points, then using a web builder may turn out to be more expensive and time-consuming than the “classy” approach

If you have answered all these questions and understand that in your case a web builder suits you, then here are their advantages:
