...
interface DatabaseViewOptions {
  ...
  // Top-level folder names (relative to the current basePath) to skip in the
  // listing. Useful at root to hide e.g. an Archive section.
  exclude: string[]
}
...
export default ((opts?: Partial<DatabaseViewOptions>) => {
  ...
  const DatabaseView: QuartzComponent = ({ allFiles, fileData }: QuartzComponentProps) => {
    const basePath = resolveBasePath(fileData.slug)
    ...
    const pages = allFiles.filter((f) => {
      ...
      // skip any page whose top-level segment under basePath is excluded
      if (excludeSet.has(parts[0])) return false
              ...
              {pages.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td>
                      {type ? (
                        <span class={`database-pill database-type type-${slugifyClass(type)}`}>
                          {type}
                        </span>
                      ) : null}
                    </td>
