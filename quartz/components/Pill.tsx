...
// Tags are free-form, so (unlike content types) we can't hand-map each to a
// hue. Derive a stable hue from the tag name instead: same tag → same color
// across every page, no upkeep. Hue classes are styled in styles/pill.scss.
const TAG_HUES = ["violet", "blue", "green", "orange", "pink", "yellow"] as const

function tagHue(tag: string): (typeof TAG_HUES)[number] {
  let h = 0
...
export function TagPillList({ tags, baseSlug }: TagPillListProps) {
  return (
    <>
      {tags.map((tag) => (
        <Pill
          className={`pill pill-tag pill-tag-${tagHue(tag)}`}
