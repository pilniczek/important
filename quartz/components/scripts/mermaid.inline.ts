...
document.addEventListener("nav", async () => {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    // Position the expand button to the left of the clipboard button. The
    // clipboard button is added by clipboard.inline.ts on the same `nav`
    // event, so it may not exist yet — defer to a microtask.
    queueMicrotask(() => {
