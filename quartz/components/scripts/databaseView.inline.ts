...
const MARKERS = [
  { slug: "archived", value: "Archived" },
  { slug: "utilities", value: "Utilities" },
] as const

function detectMarkerFromReferrer(): string | null {
  const ref = previousPath ?? document.referrer
  if (!ref) return null
  const hit = MARKERS.find((m) => ref.includes(`/types/${m.slug}`))
  return hit?.value ?? null
}

function applyMarkerContextOverride(typeSelect: HTMLSelectElement | null) {
  if (!typeSelect) return
  const marker = detectMarkerFromReferrer()
  if (!marker) return
  const hasOption = Array.from(typeSelect.options).some((o) => o.value === marker)
  if (hasOption) typeSelect.value = marker
}
...
function bind(view: HTMLElement) {
  ...
  applyTagSearch()
  // URL params take precedence over inferred marker context — an explicit
  // bookmark/back-restore should win over the referrer-based auto-select.
  const fromURL = readFiltersFromURL(controls)
  if (!fromURL) applyMarkerContextOverride(typeSelect)
