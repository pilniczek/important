...
function bind(view: HTMLElement) {
  ...
  const applyFilters = () => {
    ...
    // The second-column dropdown filters either `type` or `section`, signalled
    // by its data-filter-dim attribute (see DatabaseFilters / buildPrimaryFilter).
    const primaryDim = typeSelect?.dataset.filterDim || "type"
    const primary = (typeSelect?.value || "").toLowerCase()
    ...
    for (const row of rows) {
      ...
      const rSection = row.dataset.section || "main"
      const okName = !name || rName.includes(name)
      const okPrimary =
        !primary || (primaryDim === "section" ? rSection === primary : rTypes.includes(primary))
      const okTag = tags.length === 0 || tags.every((t) => rTags.includes(t))
      const okUrl = !url || rUrl.includes(url)
      row.style.display = okName && okPrimary && okTag && okUrl ? "" : "none"
    }
  ...
  applyTagSearch()
  // Restore any filters encoded in the URL (explicit bookmark / back-restore).
  readFiltersFromURL(controls)
