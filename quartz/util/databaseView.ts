...
function hasMarker(f: QuartzPluginData, marker: string): boolean {
  return (f.frontmatter?.type ?? []).includes(marker)
}
...
function passesFolderFilter(
  ...
  if (hasMarker(f, "Archived") || hasMarker(f, "Utilities")) return false
  if (!directChildrenOnly) return true
  const parts = f.slug!.slice(basePath.length).split("/")
  // Flat note (name.md → ["name"]) or legacy folder/index shape (name/index.md → ["name", "index"]).
  return parts.length === 1 || (parts.length === 2 && parts[1] === "index")
}

function passesTypeFilter(f: QuartzPluginData, typeSlug: string): boolean {
  // Utilities are fully siloed — only surface on /types/utilities, never on other type pages.
  if (typeSlug !== "utilities" && hasMarker(f, "Utilities")) return false
