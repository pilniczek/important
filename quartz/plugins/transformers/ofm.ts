...
export interface Options {
  ...
  mermaidQuadrantSize?: number
}

const defaultMermaidDarkColors = {
  light: "#161618",
  secondary: "#7b97aa",
  tertiary: "#84a59d",
  darkgray: "#d4d4d4",
...
let sharedMermaidRenderer: MermaidRenderer | undefined
const mermaidSvgCache = new Map<string, string>()

function buildMermaidRenderOptions(
  colors: NonNullable<Options["mermaidThemeColors"]>,
  size: number,
): RenderOptions {
  return {
    mermaidConfig: {
      ...
      // The xychart (diffusion-of-innovations.md) hides its y-axis labels/ticks and
      // x-axis ticks. This MUST happen at render time, not via CSS: CSS display:none
      // hides the groups but can't reclaim the layout space Mermaid reserves for the
      // y-axis labels, leaving a gap between the "Adopters" title and the plot. The
      // client re-render (mermaid.inline.ts) carries the same block.
      xyChart: {
        height: 250,
        xAxis: { showTick: true },
...
const defaultOptions: Options = {
  ...
  mermaidQuadrantSize: DEFAULT_QUADRANT_SIZE,
}
...
export const ObsidianFlavoredMarkdown: QuartzTransformerPlugin<Partial<Options>> = (userOpts) => {
  ...
  return {
              ...
              (value: string, ...capture: string[]) => {
                ...
                if (value.startsWith("!")) {
                  ...
                  if ([".png", ".jpg", ".jpeg", ".gif", ".bmp", ".svg", ".webp"].includes(ext)) {
                    ...
                    return {
                      type: "html",
                      value: `<iframe src="${url}#pagemode=none&view=FitH" class="pdf"></iframe>`,
                    }
                  } else {
                    const block = anchor
                    return {
                ...
                if (opts.disableBrokenWikilinks) {
                  ...
                  if (!exists) {
                    return {
                      type: "html",
                      value: `<a class=\"internal wikilink broken\">${alias ?? fp}</a>`,
                    }
                  }
                }
            ...
            replacements.push([
              ...
              (_value: string, tag: string) => {
                ...
                if (file.data.frontmatter) {
                  ...
                  const seenSlugs = new Set(noteTags.map((t: string) => slugTag(t)))
                  if (!seenSlugs.has(tagSlug)) {
                    file.data.frontmatter.tags = [...noteTags, tag]
                  }
                }
              ...
              if (node.lang === "mermaid") {
                ...
                node.data = {
                  hProperties: {
                    ...
                    "data-quadrant-size": String(
                      opts.mermaidQuadrantSize ?? DEFAULT_QUADRANT_SIZE,
                    ),
                  },
                }
              }
    ...
    htmlPlugins() {
      ...
      if (opts.mermaid) {
        const mermaidColors = opts.mermaidThemeColors ?? defaultMermaidDarkColors
        plugins.push(() => {
          return async (tree: HtmlRoot, _file) => {
            const targets: { node: Element; source: string }[] = []
            visit(tree, "element", (node: Element) => {
              ...
              const dc = (node.properties?.dataClipboard ??
                node.properties?.["data-clipboard"]) as string | undefined
              let source: string | undefined
              if (dc) {
                try {
                  source = JSON.parse(dc)
                } catch {
                  // fall through to text-child fallback
                }
              }
              if (source === undefined) {
            ...
            const uncached = targets.filter(({ source }) => !mermaidSvgCache.has(source))
            if (uncached.length > 0) {
              sharedMermaidRenderer ??= createMermaidRenderer()
              const results = await sharedMermaidRenderer(
                uncached.map(({ source }) => source),
              ...
              for (let i = 0; i < uncached.length; i++) {
                ...
                if (r.status === "fulfilled") {
                  mermaidSvgCache.set(uncached[i].source, r.value.svg)
                } else {
                  console.warn("[mermaid] render failed:", r.reason)
                }
              }
            }
            ...
            for (const { node, source } of targets) {
              ...
              node.children = parsed.children as ElementContent[]
              node.properties = node.properties ?? {}
              ;(node.properties as Record<string, unknown>).dataProcessed = "true"
            }
          }
        })
      }
