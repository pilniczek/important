...
function computeTypeInfo(
  ...
  for (const data of allFiles) {
    for (const raw of data.frontmatter?.type ?? []) {
  ...
  // Lifecycle markers get friendlier labels — see AGENTS.md "DatabaseView feature".
  ...
  const typeDescriptions: Record<string, ProcessedContent> = Object.fromEntries(
    [...types].map((ts) => {
      ...
      const title = MARKER_TITLES[raw] ?? `Type: ${raw}`
      return [
        ts,
