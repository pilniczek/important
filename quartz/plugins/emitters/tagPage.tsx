...
function computeTagInfo(
  ...
  for (const [tree, file] of content) {
    ...
    if (slug.startsWith("tags/")) {
      ...
      const displayTag = [...tags].find((t) => slugTag(t) === tagSlug) ?? tagSlug
      if (tagDescriptions[displayTag]) {
        tagDescriptions[displayTag] = [tree, file]
...
async function processTagPage(
  ...
  const slug = joinSegments("tags", tag === "index" ? "index" : slugTag(tag)) as FullSlug
  ...
  return {
    ...
    async *partialEmit(ctx, content, resources, changeEvents) {
      ...
      for (const changeEvent of changeEvents) {
        ...
        if (slug.startsWith("tags/")) {
          const tagSlug = slug.slice("tags/".length)
          const allTagsForLookup = new Set(
            allFiles.flatMap((d) => d.frontmatter?.tags ?? []).flatMap(getAllSegmentPrefixes),
          )
          const displayTag =
            [...allTagsForLookup].find((t) => slugTag(t) === tagSlug) ?? tagSlug
          affectedTags.add(displayTag)
        }
