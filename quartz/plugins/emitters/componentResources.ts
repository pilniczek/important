...
export const ComponentResources: QuartzEmitterPlugin = () => {
  return {
    ...
    async *emit(ctx, _content, _resources) {
      ...
      // ITCSS cascade layers. Declare the canonical order up front so each
      // layer's priority is fixed regardless of where its rules land in the
      // bundle. Global tiers (settings → objects) are assigned inside the
      // quartz/styles partials; every component's CSS is wrapped in `components`
      // here; custom.scss owns `overrides`. This lets low-specificity overrides
      // beat high-specificity framework rules without `!important`. See
      // AGENTS.md "Base SCSS structure".
      const layerOrder = "@layer settings, generic, elements, objects, components, overrides;"
      const componentStyles = `@layer components {\n${componentResources.css.join("\n\n")}\n}`
